...
func (r *k8sPermissionResolver) getDynamicClient(ctx context.Context) (dynamic.Interface, error) {
	if r.dynamicClient != nil {
		return r.dynamicClient, nil
	}
	restConfig, err := r.getRestConfig(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *k8sPermissionResolver) getRestConfig(ctx context.Context) (*rest.Config, error) {
	if r.restConfigProvider == nil {
		return nil, fmt.Errorf("rest config provider not available")
	}
	return r.restConfigProvider.GetRestConfig(ctx)
}
...
func (r *k8sPermissionResolver) SearchUsersPermissions(
	...
	if options.UserID > 0 {
		return r.searchSingleUserPermissions(ctx, namespace, options.UserID, options)
	}

	result, err := r.searchAllUsersPermissions(ctx, namespace, options)
	if err != nil {
		return nil, err
	}
...
func (r *k8sPermissionResolver) searchSingleUserPermissions(
	...
	dynClient, err := r.getDynamicClient(ctx)
	if err != nil {
		return nil, err
	}

	roleCache := make(map[string][]ac.Permission)
	perms, err := r.getUserPermissions(ctx, dynClient, namespace, u.UID, options, roleCache)
	if err != nil {
...
func (r *k8sPermissionResolver) searchAllUsersPermissions(
	...
	for _, userObj := range users {
		...
		perms, err := r.buildUserPermissions(ctx, dynClient, namespace, userObj, userKey, subjectRoles, roleCache, options)
		if err != nil {
...
func (r *k8sPermissionResolver) getUserPermissions(
	...
	directPerms, _ := r.getDirectResourcePermissions(ctx, namespace, userUID)
...
func (r *k8sPermissionResolver) getDirectResourcePermissions(
	...
	restConfig, err := r.getRestConfig(ctx)
	if err != nil {
		// No rest config — skip direct resource permissions gracefully.
		return nil, nil
	}
